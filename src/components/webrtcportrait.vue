  ...
  <div id="app" class="root">
    <div class="content">
      ...
      <div v-if="radio === 3" style="display: flex; margin-top: 20px">
        <el-radio-group v-model="bcRadio" style="display: flex; align-items: center" @change="switchBackground">
      ...
      <div class="container">
        <div style="text-align: center">
          原始视频
          <div class="video">
            <video ref="video" style="border: 1px solid" :class="{ flipHorizontal: isFlipHorizontal }" width="300" height="300" autoplay></video>
          </div>
        </div>
        <div style="text-align: center">
          结果视频
          <div class="video">
            <canvas id="videoCanvas" style="border: 1px solid" :class="{ flipHorizontal: isFlipHorizontal }" width="300" height="300"></canvas>
          </div>
        </div>
      </div>
      <!-- 照片 -->
...
export default {
  ...
  methods: {
    ...
    async blurBackground() {
      ...
      bodySegmentation.drawBokehEffect(this.videoCanvas, video, segmentation, this.backgroundBlurAmount, this.edgeBlurAmount, this.flipHorizontal);
      if (this.radio === 2) {
    ...
    async replaceBackground() {
      ...
      let backgroundDarkeningMask = bodySegmentation.toMask(segmentation, foregroundColor, backgroundColor);
    ...
    async loadAndPredict() {
      ...
      //加载模型
      this.segmenter = await bodySegmentation.createSegmenter(bodySegmentation.SupportedModels.BodyPix, segmenterConfig);
    },
  },
};
</script>
